  ...
  <section id="content" class="card section-shell" aria-labelledby="generator-title">
    ...
    <div class="generator-picker">
      ...
      <div ref="dropdownRef" class="generator-dropdown">
        <v-btn class="generator-dropdown-btn" variant="flat" type="button" aria-haspopup="listbox" :aria-expanded="isDropdownOpen" @click="isDropdownOpen = !isDropdownOpen">
      ...
      <div class="generator-selected-chips" aria-live="polite">
        <v-chip v-if="!selectedPolishes.length" class="generator-chip generator-chip-placeholder" size="small" variant="outlined">Selected polishes will appear here.</v-chip>
        <v-chip
          v-for="polish in selectedPolishes"
          :key="polish!.id"
    ...
    <div class="inline-controls">
      <v-btn class="btn btn-secondary" variant="flat" type="button" @click="studio.autoSuggest()">Auto-suggest</v-btn>
      <v-btn class="btn btn-primary" variant="flat" type="button" @click="studio.generateDesign()">Generate Design</v-btn>
    </div>
    ...
    <article class="design-output" aria-live="polite">
      ...
      <template v-else>
        ...
        <div class="design-save-row">
          <v-btn class="btn btn-primary" variant="flat" type="button" :disabled="studio.state.generatedResult.saved" :class="{ 'btn-saved': studio.state.generatedResult.saved }" @click="studio.saveGeneratedDesign()">{{ studio.state.generatedResult.saved ? 'Saved' : 'Save Design' }}</v-btn>
